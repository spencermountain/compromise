...
var adj = _dereq_('../subset/adjectives/methods');
var toPlural = _dereq_('../subset/nouns/methods/pluralize');
var fastConjugate = _dereq_('../subset/verbs/methods/conjugate/faster');
...
var buildOut = function buildOut(lex, options) {
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    if (lex[str] === 'PhrasalVerb') {
      var parts = str.split(/ /);
      var _obj = fastConjugate(parts[0]);
      var _tags = Object.keys(_obj);
      for (var _o = 0; _o < _tags.length; _o++) {
        var _tag = _tags[_o];
        lex[_obj[_tag] + ' ' + parts[1]] = 'PhrasalVerb';
      }
      continue;
    }
  }
...
var toNegative = function toNegative(ts) {
  ...
  if (past.found) {
    ...
    first.terms[0].text = toInfinitive(first.terms[0]);
  ...
  if (pres.found) {
    ...
    _first.terms[0].text = toInfinitive(_first.terms[0]);
  ...
  if (verb.found) {
    ...
    _first2.terms[0].text = toInfinitive(_first2.terms[0]);
...
var findMultiWords = function findMultiWords(ts, i, world) {
  var want = world.firstWords[ts.terms[i].normal];
  var str = '';
  ...
  for (var add = 1; add <= 3; add++) {
    ...
    //perfect match here?
    if (want[str] === true) {
      var tag = world.words[ts.terms[i].normal + ' ' + str];
...
var lexiconMulti = function lexiconMulti(ts) {
  var firstWords = ts.world.firstWords || {};
  for (var i = 0; i < ts.terms.length; i++) {
    var t = ts.terms[i];
  ...
  var methods = {
    ...
    replace: function replace(str1, str2, keepTags) {
      ...
      if (str2.match(/\$1\b/)) {
        ...
        var captureThis = str1.match(/\[(.+?)\]/);
        if (captureThis && captureThis[1]) {
          var found = this.match(str1).match(captureThis[1]);
...
//in World.proto..
var addConjugations = function addConjugations(conjugations) {
  var _this = this;

  Object.keys(conjugations || {}).forEach(function (k) {
    _this.conjugations[k] = conjugations[k];
    //add them both to the lexicon, too
    // let plural = plurals[k];
    _this.words[k] = _this.words[k] || 'Infinitive';
...
//in World.proto..
var addPlurals = function addPlurals(plurals) {
  var _this = this;

  Object.keys(plurals || {}).forEach(function (k) {
    _this.plurals[k] = plurals[k];
    //add them both to the lexicon, too
    var plural = plurals[k];
    _this.words[k] = _this.words[k] || 'Singular';
    _this.words[plural] = _this.words[plural] || 'Plural';
  });
};
...
var addDownward = _dereq_('../tagset/addDownward');
...
//basically really dirty and stupid.
var unpackLex = function unpackLex(lex) {
  lex = lex || {};
...
var addWords = function addWords(lex) {
  var _this = this;
...
var tagset = _dereq_('../tagset');
var regex = _dereq_('./rules');
var plurals = _dereq_('../lexicon/uncompressed/irregularPlurals').toPlural;
