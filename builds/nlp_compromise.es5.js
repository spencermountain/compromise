...
var verbMap={infinitive:'Infinitive',past:'PastTense',present:'PresentTense',future:'FutureTense',actor:'Actor',gerund:'Gerund'};var verbs=require('./data/verbs.js');var _loop=function(_i9){var o=verb_conjugate(verbs[_i9]);Object.keys(o).forEach(function(k){if(k && o[k]){lexicon[o[k]] = verbMap[k];}});};for(var _i9=0;_i9 < verbs.length;_i9++) {_loop(_i9);} //irregular verbs
...
}}return terms;}};module.exports = {easy_ones:easy_ones,hard_ones:hard_ones};},{"../../sentence/pos/pos.js":26}],24:[function(require,module,exports){ //fancy combining/chunking of terms
...
// },
//
for(var _i13=0;_i13 < lump_rules.length;_i13++) {if(lump_rules[_i13].condition){return lump_rules[_i13].result;}}return false;};var lump=function lump(a,b,tag){var c=pos.classMapping[tag] || pos.Term;var t=new c(a.text + ' ' + b.text,tag);t.reason = 'lumped(' + t.reason + ')';return t;};var fancy_lumping=function fancy_lumping(terms){for(var _i14=1;_i14 < terms.length;_i14++) {var a=terms[_i14 - 1];var b=terms[_i14]; // Person, Capital -> "Person Capital"
var l=shouldLump(a,b);if(l !== false){terms[_i14] = lump(a,b,l);terms[_i14 - 1] = null;continue;}} //remove killed terms
...
for(var _i15=1;_i15 < terms.length;_i15++) {if(terms[_i15].is_capital() || terms[_i15].is_acronym()){terms[_i15] = assign(terms[_i15],'Noun','capital_signal');}}return terms;}; //regex hints for words/suffixes
var word_rules_pass=function word_rules_pass(terms){for(var _i16=0;_i16 < terms.length;_i16++) {if(terms[_i16].tag !== '?'){continue;}for(var o=0;o < word_rules.length;o++) {if(terms[_i16].normal.length > 4 && terms[_i16].normal.match(word_rules[o].reg)){terms[_i16] = assign(terms[_i16],word_rules[o].pos,'rules_pass_' + o);break;}}}return terms;}; //turn [noun, noun..] into [noun..]
var chunk_neighbours=function chunk_neighbours(terms){var new_terms=[];var last=null;for(var _i17=0;_i17 < terms.length;_i17++) {var t=terms[_i17];if(last !== null && t.tag === last){new_terms[new_terms.length - 1].text += ' ' + t.text;new_terms[new_terms.length - 1].normalize();}else {new_terms.push(t);}last = t.tag;}return new_terms;}; //hints from the sentence grammar
var grammar_rules_pass=function grammar_rules_pass(s){var tags=s.tags();for(var _i18=0;_i18 < s.terms.length;_i18++) {var t=s.terms[_i18];for(var o=0;o < grammar_rules.length;o++) {var rule=grammar_rules[o]; //does this rule match
if(fns.sameArr(rule.before,tags.slice(_i18,_i18 + rule.before.length))){ //change before/after for each term
for(var c=0;c < rule.before.length;c++) {s.terms[_i18 + c] = assign(s.terms[_i18 + c],rule.after[c],'grammar_rule ' + c);}}}}return s.terms;};var noun_fallback=function noun_fallback(terms){for(var _i19=0;_i19 < terms.length;_i19++) {if(terms[_i19].tag === '?' && terms[_i19].normal.match(/[a-z]/)){terms[_i19] = assign(terms[_i19],'Noun','fallback');}}return terms;}; //turn nouns into person/place
var specific_pos=function specific_pos(terms){for(var _i20=0;_i20 < terms.length;_i20++) {var t=terms[_i20];if(t instanceof pos.Noun){if(t.is_person()){terms[_i20] = assign(t,'Person');}if(t.is_place()){terms[_i20] = assign(t,'Place');}if(t.is_value()){terms[_i20] = assign(t,'Value');}if(t.is_date()){terms[_i20] = assign(t,'Date');}}}return terms;};var tagger=function tagger(s){ //word-level rules
...
for(var _i21=0;_i21 < 2;_i21++) {s.terms = grammar_rules_pass(s);s.terms = chunk_neighbours(s.terms);s.terms = noun_fallback(s.terms);s.terms = specific_pos(s.terms);s.terms = contractions.hard_ones(s.terms);s.terms = fancy_lumping(s.terms);}return s.terms;};module.exports = tagger;},{"../../fns":20,"../../lexicon.js":22,"./contractions":23,"./fancy_lumping":24,"./grammar_rules":25,"./pos":26,"./word_rules":28}],28:[function(require,module,exports){var tag_mapping=require('./pos.js').tag_mapping; //regex patterns and parts of speech],
...
for(var _i34=0;_i34 < a_regexs.length;_i34++) {if(str.match(a_regexs[_i34])){return 'a';}} //basic vowel-startings
...
for(var _i35=0;_i35 < irregulars.length;_i35++) {if(irregulars[_i35][1] === str){return true;}if(irregulars[_i35][0] === str){return false;}}for(var _i36=0;_i36 < plural_indicators.length;_i36++) {if(str.match(plural_indicators[_i36])){return true;}}for(var _i37=0;_i37 < singular_indicators.length;_i37++) {if(str.match(singular_indicators[_i37])){return false;}} // some 'looks pretty plural' rules
...
for(var _i39=0;_i39 < pluralize_rules.length;_i39++) {if(str.match(pluralize_rules[_i39].reg)){return str.replace(pluralize_rules[_i39].reg,pluralize_rules[_i39].repl);}}}; // console.log(pluralize('gas') === "gases")
...
for(var _i40=0;_i40 < singularize_rules.length;_i40++) {if(str.match(singularize_rules[_i40].reg)){return str.replace(singularize_rules[_i40].reg,singularize_rules[_i40].repl);}}return str;}; // console.log(singularize('gases') === "gas")
...
var mults=[{reg:/^(minus|negative)[\s\-]/i,mult:-1},{reg:/^(a\s)?half[\s\-](of\s)?/i,mult:0.5},{reg:/^(a\s)?quarter[\s\-]/i,mult:0.25}];for(var _i42=0;_i42 < mults.length;_i42++) {if(s.match(mults[_i42].reg)){global_multiplier = mults[_i42].mult;s = s.replace(mults[_i42].reg,'');break;}} //do each word in turn..
var words=s.toString().split(/[\s\-]+/);var w=undefined,x=undefined;var current_sum=0;var local_multiplier=1;var decimal_mode=false;for(var _i43=0;_i43 < words.length;_i43++) {w = words[_i43]; //skip 'and' eg. five hundred and twelve
...
s = s.replace(/s$/); //ew
if(units[s]){return true;}return false;}},{key:"parse",value:function parse(){var words=this.normal.split(' ');var numbers='';var unit='';for(var _i44=0;_i44 < words.length;_i44++) {var w=words[_i44];if(nums.ones[w] || nums.teens[w] || nums.tens[w] || nums.multiples[w]){numbers += ' ' + w;}else if(this.is_unit(w)){ //optional units come after the number
...
arr = arr.map(function(w){return w.trim();});if(arr.length > 2){return arr;}var ones=[/^[^aeiou]?ion/,/^[^aeiou]?ised/,/^[^aeiou]?iled/];var l=arr.length;if(l > 1){var suffix=arr[l - 2] + arr[l - 1];for(var _i45=0;_i45 < ones.length;_i45++) {if(suffix.match(ones[_i45])){arr[l - 2] = arr[l - 2] + arr[l - 1];arr.pop();}}}return arr;}var syllables=function syllables(str){var all=[]; //method is nested because it's called recursively
var doer=function doer(_x6){var _again4=true;_function4: while(_again4) {var w=_x6;vow = chars = before = after = current = _i46 = candidate = undefined;_again4 = false;var vow=/[aeiouy]$/;var chars=w.split('');var before='';var after='';var current='';for(var _i46=0;_i46 < chars.length;_i46++) {before = chars.slice(0,_i46).join('');current = chars[_i46];after = chars.slice(_i46 + 1,chars.length).join('');var candidate=before + chars[_i46]; //it's a consonant that comes after a vowel
...
var obj={};var l=verb_irregulars.length;for(var _i47=0;_i47 < l;_i47++) {var x=verb_irregulars[_i47];if(verb === x.present || verb === x.gerund || verb === x.past || verb === x.infinitive){obj = JSON.parse(JSON.stringify(verb_irregulars[_i47])); // object 'clone' hack, to avoid mem leak
...
l = verb_rules[predicted].length;for(var _i48=0;_i48 < l;_i48++) {var r=verb_rules[predicted][_i48];if(w.match(r.reg)){obj[predicted] = w;var keys=Object.keys(r.repl);for(var o=0;o < keys.length;o++) {if(keys[o] === predicted){obj[keys[o]] = w;}else {obj[keys[o]] = w.replace(r.reg,r.repl[keys[o]]);}}return fufill(obj);}} //produce a generic transformation
...
for(var _i52=1;_i52 <= max_size;_i52++) {keys.push({});} // Create a hash for counting..
var textlen=terms.length;for(var _i53=0;_i53 < textlen;_i53++) {var s=terms[_i53];keys[1][s] = (keys[1][s] || 0) + 1;for(var j=2;j <= max_size;j++) {if(_i53 + j <= textlen){s += ' ' + terms[_i53 + j - 1];keys[j][s] = (keys[j][s] || 0) + 1;}else {break;}}} // map the hash to an array for sorting
for(var k=1;k < max_size;k++) {results[k] = [];var key=keys[k];var words=Object.keys(keys[k]);for(var _i54=0;_i54 < words.length;_i54++) {var word=words[_i54];if(key[word] >= min_count){results[k].push({'word':word,'count':key[word],'size':k});}}} //post-process + sort
...
var chunks_length=chunks.length;for(var _i55=0;_i55 < chunks_length;_i55++) {if(chunks[_i55]){ //trim whitespace
chunks[_i55] = chunks[_i55].replace(/^\s+|\s+$/g,''); //should this chunk be combined with the next one?
if(chunks[_i55 + 1] && chunks[_i55].match(abbrev_reg) || chunks[_i55].match(acronym_reg) || chunks[_i55].match(elipses_reg)){chunks[_i55 + 1] = ((chunks[_i55] || '') + ' ' + (chunks[_i55 + 1] || '')).replace(/ +/g,' ');}else if(chunks[_i55] && chunks[_i55].length > 0){ //this chunk is a proper sentence..
sentences.push(chunks[_i55]);chunks[_i55] = '';}}} //if we never got a sentence, return the given text
