...
var verbs=require('./data/verbs.js');for(var _i8=0;_i8 < verbs.length;_i8++) {var c=verb_conjugate(verbs[_i8]);lexicon[c.infinitive] = 'VBP';lexicon[c.past] = 'VBD';lexicon[c.gerund] = 'VBG';lexicon[c.present] = 'VBZ';if(c.doer){lexicon[c.doer] = 'NNA';}if(c.participle){lexicon[c.participle] = 'VBN';}}addArr(require('./data/abbreviations.js').abbreviations,'NNAB');addArr(require('./data/adjectives.js'),'JJ');addArr(require('./data/demonyms.js'),'JJ');addArr(require('./data/honourifics.js'),'NNAB');addArr(require('./data/uncountables.js'),'NN');addArr(require('./data/dates.js'),'CD');addArr(require('./data/numbers.js'),'CD'); //a little fancy
...
var assign=function assign(t,tag,reason){var P=mapping[tag] || Term;t = new P(t.text,tag);t.reason = reason;return t;};module.exports = assign;},{"./pos":25}],22:[function(require,module,exports){ //add a 'quiet' token for contractions so we can represent their grammar
...
{'before':['Modal','Value'],'after':['Modal','Verb']},{'before':['Adverb','Value'],'after':['Adverb','Verb']}];},{}],24:[function(require,module,exports){'use strict';var assign=require('./assign');var lexicon=require('../../lexicon.js'); //consult lexicon for this known-word
...
for(var _i10=1;_i10 < terms.length;_i10++) {if(terms[_i10].is_capital() || terms[_i10].is_acronym()){terms[_i10] = assign(terms[_i10],'Noun','capital_signal');}}return terms;}; //regex hints for words/suffixes
...
var chunk_neighbours=function chunk_neighbours(terms){var new_terms=[];var last=null;for(var _i12=0;_i12 < terms.length;_i12++) {var t=terms[_i12];if(last !== null && t.pos === last){new_terms[new_terms.length - 1].text += ' ' + t.text;new_terms[new_terms.length - 1].normalize();}else {new_terms.push(t);}last = t.pos;}return new_terms;}; //hints from the sentence grammar
...
for(var c=0;c < rule.before.length;c++) {s.terms[_i13 + c] = assign(s.terms[_i13 + c],rule.after[c],'grammar_rule ' + c);}}}}return s.terms;};var noun_fallback=function noun_fallback(terms){for(var _i14=0;_i14 < terms.length;_i14++) {if(terms[_i14].pos === '?'){terms[_i14] = assign(terms[_i14],'Noun','fallback');}}return terms;}; //turn nouns into person/place
var specific_pos=function specific_pos(terms){for(var _i15=0;_i15 < terms.length;_i15++) {var t=terms[_i15];if(t instanceof pos.Noun){if(t.is_person()){terms[_i15] = assign(t,'Person');}if(t.is_place()){terms[_i15] = assign(t,'Place');}if(t.is_value()){terms[_i15] = assign(t,'Value');}if(t.is_date()){terms[_i15] = assign(t,'Date');}}}return terms;};var tagger=function tagger(s){s.terms = capital_signals(s.terms);s.terms = contractions(s.terms);s.terms = lexicon_pass(s.terms);s.terms = word_rules_pass(s.terms); //repeat these two
...
s.terms = specific_pos(s.terms);return s.terms;};module.exports = tagger;},{"../../fns":18,"./assign":21,"./contractions":22,"./grammar_rules":23,"./lexicon_pass":24,"./pos":25,"./word_rules":27}],27:[function(require,module,exports){ //regex patterns and parts of speech],
...
for(var _i29=0;_i29 < irregulars.length;_i29++) {if(irregulars[_i29][1] === str){return true;}if(irregulars[_i29][0] === str){return false;}}for(var _i30=0;_i30 < plural_indicators.length;_i30++) {if(str.match(plural_indicators[_i30])){return true;}}for(var _i31=0;_i31 < singular_indicators.length;_i31++) {if(str.match(singular_indicators[_i31])){return false;}} // some 'looks pretty plural' rules
...
module.exports = is_plural;},{"../../data/irregular_nouns":9}],44:[function(require,module,exports){ //uncountables are words that shouldn't ever inflect, for metaphysical reasons, like 'peace'
...
}}]);return Person;})(Noun);module.exports = Person;},{"../../../data/firstnames":7,"../../../data/honourifics":8,"../noun.js":45}],50:[function(require,module,exports){'use strict';var places=require('../../../data/places');var abbreviations=require('../../../data/abbreviations'); //add Country names
...
for(var _i33=0;_i33 < pluralize_rules.length;_i33++) {if(str.match(pluralize_rules[_i33].reg)){return str.replace(pluralize_rules[_i33].reg,pluralize_rules[_i33].repl);}}}; // console.log(pluralize('gas') === "gases")
...
for(var _i34=0;_i34 < singularize_rules.length;_i34++) {if(str.match(singularize_rules[_i34].reg)){return str.replace(singularize_rules[_i34].reg,singularize_rules[_i34].repl);}}return str;}; // console.log(singularize('gases') === "gas")
...
var mults=[{reg:/^(minus|negative)[\s\-]/i,mult:-1},{reg:/^(a\s)?half[\s\-](of\s)?/i,mult:0.5},{reg:/^(a\s)?quarter[\s\-]/i,mult:0.25}];for(var _i36=0;_i36 < mults.length;_i36++) {if(s.match(mults[_i36].reg)){global_multiplier = mults[_i36].mult;s = s.replace(mults[_i36].reg,'');break;}} //do each word in turn..
var words=s.toString().split(/[\s\-]+/);var w=undefined,x=undefined;var current_sum=0;var local_multiplier=1;var decimal_mode=false;for(var _i37=0;_i37 < words.length;_i37++) {w = words[_i37]; //skip 'and' eg. five hundred and twelve
...
s = s.replace(/s$/); //ew
if(units[s]){return true;}return false;}},{key:"parse",value:function parse(){var words=this.normal.split(' ');var numbers='';var unit='';for(var _i38=0;_i38 < words.length;_i38++) {var w=words[_i38];if(nums.ones[w] || nums.teens[w] || nums.tens[w] || nums.multiples[w]){numbers += ' ' + w;}else if(this.is_unit(w)){ //optional units come after the number
...
arr = arr.map(function(w){return w.trim();});if(arr.length > 2){return arr;}var ones=[/^[^aeiou]?ion/,/^[^aeiou]?ised/,/^[^aeiou]?iled/];var l=arr.length;if(l > 1){var suffix=arr[l - 2] + arr[l - 1];for(var _i39=0;_i39 < ones.length;_i39++) {if(suffix.match(ones[_i39])){arr[l - 2] = arr[l - 2] + arr[l - 1];arr.pop();}}}return arr;}var syllables=function syllables(str){var all=[]; //method is nested because it's called recursively
var doer=function doer(_x5){var _again3=true;_function3: while(_again3) {var w=_x5;vow = chars = before = after = current = _i40 = candidate = undefined;_again3 = false;var vow=/[aeiouy]$/;var chars=w.split('');var before='';var after='';var current='';for(var _i40=0;_i40 < chars.length;_i40++) {before = chars.slice(0,_i40).join('');current = chars[_i40];after = chars.slice(_i40 + 1,chars.length).join('');var candidate=before + chars[_i40]; //it's a consonant that comes after a vowel
...
'use strict';var verb_to_actor=require('./to_actor');var verb_irregulars=require('./verb_irregulars');var verb_rules=require('./verb_rules');var predict=require('./predict_form.js'); //fallback to this transformation if it has an unknown prefix
...
delete result['actor'];Object.keys(result).forEach(function(k){if(result[k]){result[k] += ' ' + particle;}});return {v:result};})();if(typeof _ret === "object")return _ret.v;} //for pluperfect ('had tried') remove 'had' and call it past-tense
...
var obj={};var l=verb_irregulars.length;for(var _i41=0;_i41 < l;_i41++) {var x=verb_irregulars[_i41];if(verb === x.present || verb === x.gerund || verb === x.past || verb === x.infinitive){obj = JSON.parse(JSON.stringify(verb_irregulars[_i41])); // object 'clone' hack, to avoid mem leak
...
l = verb_rules[predicted].length;for(var _i42=0;_i42 < l;_i42++) {var r=verb_rules[predicted][_i42];if(w.match(r.reg)){obj[predicted] = w;var keys=Object.keys(r.repl);for(var o=0;o < keys.length;o++) {if(keys[o] === predicted){obj[keys[o]] = w;}else {obj[keys[o]] = w.replace(r.reg,r.repl[keys[o]]);}}return fufill(obj);}} //produce a generic transformation
...
},{"./predict_form.js":62,"./to_actor":64,"./verb_irregulars":65,"./verb_rules":66}],62:[function(require,module,exports){'use strict'; //this method is used to predict which current conjugation a verb is
...
var predict=function predict(w){var arr=Object.keys(suffix_rules);for(var _i43=0;_i43 < arr.length;_i43++) {if(fns.endsWith(w,arr[_i43])){return suffix_rules[arr[_i43]];}}return 'infinitive';};module.exports = predict;},{"../../../fns.js":18,"./suffix_rules":63}],63:[function(require,module,exports){'use strict'; //generated from test data
var compact={'gerund':['ing'],'infinitive':['ate','ize','tion','rify','ress','ify','age','nce','ect','ise','ine','ish','ace','ash','ure','tch','end','ack','and','ute','ade','ock','ite','ase','ose','use','ive','int','nge','lay','est','ain','ant','eed','er','le'],'past':['ed','lt','nt','pt','ew','ld'],'present':['rks','cks','nks','ngs','mps','tes','zes','ers','les','acks','ends','ands','ocks','lays','eads','lls','els','ils','ows','nds','ays','ams','ars','ops','ffs','als','urs','lds','ews','ips','es','ts','ns','s']};var suffix_rules={};var keys=Object.keys(compact);var l=keys.length;var l2=undefined;for(var _i44=0;_i44 < l;_i44++) {l2 = compact[keys[_i44]].length;for(var o=0;o < l2;o++) {suffix_rules[compact[keys[_i44]][o]] = keys[_i44];}}module.exports = suffix_rules;},{}],64:[function(require,module,exports){ //somone who does this present-tense verb
//turn 'walk' into 'walker'
'use strict';var actor=function actor(str){str = str || '';var irregulars={'tie':'tier','dream':'dreamer','sail':'sailer','run':'runner','rub':'rubber','begin':'beginner','win':'winner','claim':'claimant','deal':'dealer','spin':'spinner'};var dont={'aid':1,'fail':1,'appear':1,'happen':1,'seem':1,'try':1,'say':1,'marry':1,'be':1,'forbid':1,'understand':1,'bet':1};var transforms=[{'reg':/e$/i,'repl':'er'},{'reg':/([aeiou])([mlgp])$/i,'repl':'$1$2$2er'},{'reg':/([rlf])y$/i,'repl':'$1ier'},{'reg':/^(.?.[aeiou])t$/i,'repl':'$1tter'}];if(dont.hasOwnProperty(str)){return null;}if(irregulars.hasOwnProperty(str)){return irregulars[str];}for(var _i45=0;_i45 < transforms.length;_i45++) {if(str.match(transforms[_i45].reg)){return str.replace(transforms[_i45].reg,transforms[_i45].repl);}}return str + 'er';}; // console.log(verb_to_actor('set'))
// console.log(verb_to_actor('sweep'))
// console.log(verb_to_actor('watch'))
module.exports = actor;},{}],65:[function(require,module,exports){'use strict';var types=['infinitive','gerund','past','present','doer','future']; //list of verb irregular verb forms, compacted to save space. ('_' -> infinitive )
...
module.exports = compact.map(function(arr){var obj={};for(var _i46=0;_i46 < arr.length;_i46++) {obj[types[_i46]] = arr[_i46].replace(/_/,arr[0]);}return obj;}); // console.log(JSON.stringify(verb_irregulars, null, 2));
},{}],66:[function(require,module,exports){ // regex rules for each part of speech that convert it to all other parts of speech.
...
},{key:"isNegative",value:function isNegative(){var str=this.normal;if(str.match(/(n't|\bnot\b)/)){return true;}return false;}},{key:"negate",value:function negate(){if(this.isNegative()){return this.text;}this.changeTo(_negate(this));return this.text;}}]);return Verb;})(Term);module.exports = Verb;},{"../term.js":60,"./conjugate/conjugate.js":61,"./conjugate/predict_form.js":62,"./negate.js":67}],69:[function(require,module,exports){'use strict'; //split a string into all possible parts
...
module.exports = ngram;},{"../fns.js":18}],70:[function(require,module,exports){ //(Rule-based sentence boundary segmentation) - chop given text into its proper sentences.
