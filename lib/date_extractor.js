...
exports.date_extractor=function(text){
 ...
 if(text==null){return '';}
  ...
  var second =  /[0-9]{1,2}(th|rd|st)? ?( to |-| until |\/) ?[0-9]{1,2}(th|rd|st)? (of )?(july|august|september|october|november|december|january|february|march|april|may|june)( |, |,)[0-9]{4}/i.exec(sentences[i]);       
    ...
    foundarray[0]=found;
        ...
        start=start.replace('of','');end=end.replace('of','');
    foundarray[1]= start;
        foundarray[2]= end;
       return foundarray;
    }
...
function formatdate(found){ 
    if(!found){return null;}
      ...
      if (month=='december'){monthnum='12';} 
        ...
        if(found.match('B.C.')){var year = /[0-9]{3,4}/i.exec(found);  year=year+'';
                                if(year.length==3){year='0'+year;}
                                if(year.length==2){year='00'+year;}
                                year='-'+year;
                                return year;}//something bc
        else{ var year = /[0-9]{4}/i.exec(found);   }//normal years
      year=''+year;
      found=found.replace(year,'');
      var date = /[0-9]{1,2}/i.exec(found);  
        
        if(date!=null){ //acre.write('kkkkk' + date + 'lll');
        if(date<10){date='0'+date; } //turn 1 into 01
          }

      var mqldate='';
      if(date!=null&&monthnum!=0&&year!=null){ mqldate= year+ '-' +monthnum+ '-' +date;} //all 3
      else{ if(monthnum!=0&&year!=null){ mqldate= year+ '-' +monthnum;}     //year and month
            else{ if(year!=null){ mqldate= ''+year;} //just year
                }
            }

      mqldate=mqldate.replace(' ','');
          /////////
     return 'date='+mqldate;
}
