...
const dateTagger = function(doc) {
  ...
  if (doc.has('#Date')) {
    //friday to sunday
    doc.match('#Date #Preposition #Date').tag('Date', here$1);
    //once a day..
    doc.match('(once|twice) (a|an|each) #Date').tag('Date', here$1);
    //TODO:fixme
    doc.match('(by|until|on|in|at|during|over|every|each|due) the? #Date').tag('Date', here$1);
    //tuesday
    doc.match('#Date+').tag('Date', here$1);
    //by June
    doc.match('(by|until|on|in|at|during|over|every|each|due) the? #Date').tag('Date', here$1);
    //a year after..
    doc.match('a #Duration').tag('Date', here$1);
    //between x and y
    doc.match('(between|from) #Date').tag('Date', here$1);
    doc.match('(to|until|upto) #Date').tag('Date', here$1);
    doc.match('#Date and #Date').tag('Date', here$1);
    //during this june
    doc.match('(by|until|after|before|during|on|in|following) (next|this|last)? (#Date|#Date)').tag('Date', here$1);
    //day after next
    doc.match('the? #Date after next one?').tag('Date', here$1);
    //approximately...
...
const here$2 = 'section-tagger';
//
const sectionTagger = function(doc) {
  if (doc.has('#Date')) {
    // //next september
    doc.match('this? (last|next|past|this|previous|current|upcoming|coming|the) #Date').tag('Date', here$2);
    //starting this june
    doc.match('(starting|beginning|ending) #Date').tag('Date', here$2);
    //start of june
    doc.match('the? (start|end|middle|beginning) of (last|next|this|the) (#Date|#Date)').tag('Date', here$2);
    //this coming june
...
const here$4 = 'shift-tagger';
//
const shiftTagger = function(doc) {
  if (doc.has('#Date')) {
    //two weeks before
    doc.match('#Cardinal #Duration (before|after)').tag('#DateShift', here$4);
...
const fixUp = function(doc) {
  ...
  if (doc.has('#Date')) {
    ...
    if (d.has(knownDate)) {
      //yesterday 7
      d.match(`${knownDate} #Value$`).terms(1).unTag('Date', here$5);
      //7 yesterday
      d.match(`^#Value ${knownDate}$`).terms(0).unTag('Date', here$5);
      //friday yesterday
      d.match(`#WeekDay+ ${knownDate}$`).unTag('Date').lastTerm().tag('Date', here$5);
      d.match(`${knownDate}+ ${knownDate}$`).unTag('Date').lastTerm().tag('Date', here$5);
      d.match(`(this|last|next) #Date ${knownDate}$`).unTag('Date').lastTerm().tag('Date', here$5);
    }
    //tomorrow on 5
    d.match(`on #Cardinal$`).unTag('Date', here$5);
    //this tomorrow
    d.match(`this tomorrow`).terms(0).unTag('Date', here$5);
    //q2 2019
    d.match(`(q1|q2|q3|q4) #Year`).tag('Date', here$5);
    //5 tuesday
    ...
    if (d.has('(last|this|next)')) {
      //this month 7
      d.match(`(last|this|next) #Duration #Value`).terms(2).unTag('Date', here$5);
      //7 this month
...
const parseTime$1 = function(doc) {
...
class Day extends Unit_1 {
  constructor(str, unit) {
    super(str, unit);
    this.unit = 'day';
  }
}
class Month extends Unit_1 {
  constructor(str, unit) {
    super(str, unit);
    this.unit = 'month';
  }
}
class Quarter extends Unit_1 {
  constructor(str, unit) {
    super(str, unit);
    this.unit = 'quarter';
  }
}
class Year extends Unit_1 {
  constructor(str, unit) {
    super(str, unit);
    this.unit = 'year';
  }
}
class WeekDay extends Unit_1 {
  constructor(str, unit) {
    super(str, unit);
    this.unit = 'week';
    this.d = this.d.day(str);
    //assume a wednesday in the future
    if (this.d.date() < require$$0.now().date()) {
...
// like 'feb 2'
class CalendarDate extends Unit_1 {
  constructor(str, unit) {
    super(str, unit);
    this.unit = 'day';
  }
...
const namedUnit = function(doc) {
  ...
  if (m.found === true) {
    let str = m.lastTerm().out('reduced');
    let unit = new units$4.WeekDay(str);
    return unit
  }
  return null
};
...
const { Unit: Unit$2 } = units$4;

const parseDate = function(doc) {
  let shift = _01Shift(doc);
  let time = _02Time(doc);
  let rel = _03Relative(doc);
  let d = null;
  // let str = doc.text('reduced')
...
const addMethods$5 = function(Doc, world) {
  // our new tags
  world.addTags(tags);
