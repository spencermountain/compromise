...
test('set today context', function (t) {
  ...
  t.equal(json.start, '1996-03-28T00:00:00.000-04:00', '+5hrs')

  json = doc.dates({ today: '1996-11-28', timezone: 'Canada/Eastern' }).json()[0]
  t.equal(json.start, '1996-11-28T00:00:00.000-05:00', '+5hrs')

  doc = nlp('in 3 weeks')
  json = doc.dates({ today: '1996-03-1', timezone: 'Canada/Eastern' }).json()[0]
  t.equal(json.start, '1996-03-22T00:00:00.000-04:00', 'today-start')
  t.equal(json.end, '1996-03-22T23:59:59.999-04:00', 'today-end')
  t.end()
})
