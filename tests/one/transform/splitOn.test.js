...
test('one split, one sentence', function (t) {
  ...
  t.equal(m.eq(1).out('normal'), 'match', 'found match')
  t.equal(m.eq(2).out('normal'), 'after after', 'found after')
  t.end()
})
...
test('multi split, one sentence', function (t) {
  ...
  t.equal(m.eq(0).out('normal'), 'before before', 'found before')
  t.equal(m.eq(1).out('normal'), 'match', 'found match')
  t.equal(m.eq(2).out('normal'), 'then a', 'found between')
  t.equal(m.eq(3).out('normal'), 'match', 'found match2')
  t.equal(m.eq(4).out('normal'), 'after after', 'found after')
  t.end()
})
...
test('one split, multi sentence', function (t) {
  ...
  t.equal(m.eq(1).out('normal'), 'match', 'found match')
  t.equal(m.eq(2).out('normal'), 'after after', 'found after')
  t.equal(m.eq(3).out('normal'), 'then over here', 'next sentence')
  t.end()
})
...
test('multi split, multi sentence', function (t) {
  ...
  t.equal(m.eq(2).out('normal'), 'match2', 'found match2')
  t.equal(m.eq(3).out('normal'), 'after after', 'found after')
  t.equal(m.eq(4).out('normal'), 'then a', 'next sentence')
  t.equal(m.eq(5).out('normal'), 'match3', 'next sentence match')
  t.equal(m.eq(6).out('normal'), 'over here', 'next sentence after')
  t.end()
})
...
test('greedy split', function (t) {
  ...
  let m = doc.splitOn('match+')
  t.equal(m.length, 4, 'found 4')
  t.equal(m.eq(0).out('normal'), 'match match', 'found two')
  t.equal(m.eq(1).out('normal'), 'middle middle', 'found middles')
  t.equal(m.eq(2).out('normal'), 'match', 'found one')
  t.equal(m.eq(3).out('normal'), 'then over here', 'next sentence')
  t.end()
})
...
test('split skip sentence', function (t) {
  ...
  t.equal(m.eq(0).out('normal'), 'before', 'found before')
  t.equal(m.eq(1).out('normal'), 'match', 'found match')
  t.equal(m.eq(2).out('normal'), 'nothing found here', 'no-match sentence')
  t.equal(m.eq(3).out('normal'), 'two', 'found before2')
  t.equal(m.eq(4).out('normal'), 'match', 'found match2')
  t.equal(m.eq(5).out('normal'), 'after', 'found after')
  t.end()
})
...
test('no match split', function (t) {
  ...
  t.equal(m.eq(0).text(), 'nothing found here.', 'not found 1')
  t.equal(m.eq(1).text(), 'none here either', 'not found 2')
  t.end()
})
